-- Dokumenty "pro všechny" (target_type = vsichni) mohou být zobrazeny i CP, pokud visible_to_cp = true
ALTER TABLE public.documents ADD COLUMN IF NOT EXISTS visible_to_cp BOOLEAN NOT NULL DEFAULT false;

-- Aktualizace get_person_documents: CP vidí i dokumenty vsichni s visible_to_cp = true
...
BEGIN
    ...
    IF v_person_type = 'cp' THEN
        -- CP vidí: skupina CP + dokumenty pro tuto osobu + dokumenty "všichni" s visible_to_cp = true
        RETURN QUERY
        SELECT d.id, d.title, d.content, d.doc_type, d.target_type, d.sort_order, d.priority
        FROM public.documents d
        WHERE d.larp_id = v_larp_id
        AND (
            (d.target_type = 'skupina' AND d.target_group = 'CP')
            OR (d.target_type = 'osoba' AND d.target_person_id = p_person_id)
            OR (d.target_type = 'vsichni' AND d.visible_to_cp = true)
        )
