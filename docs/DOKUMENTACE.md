...
- **verify_person_access(p_access_token UUID, p_password TEXT)**  
  Volá se z portálu při zadání hesla. Kontroluje `persons.access_token` a heslo (crypt). Vrací jeden řádek: person_id, person_name, person_type, run_id, larp_name, run_name, mission_briefing, group_name, performer, performance_times (Act Info pro CP), run_contact, run_footer_text (zápatí), larp_theme. **SECURITY DEFINER** – nevyžaduje auth, vystavuje jen data dané osoby.
...
- **Hráč/CP:** Nepřihlašují se do Supabase Auth. Přístup k datům pouze přes RPC **verify_person_access** a **get_person_documents** po znalosti access_token a hesla. Žádné přímé čtení tabulek.
...
5. **Postavy / CP** – přidání osob, nastavení hesla, zobrazení/zkopírování linku (`/portal/:accessToken`).
...
1. Otevření linku `/portal/:token` (nebo alias `/hrac/:token`).
2. Pokud není session: zobrazení formuláře s heslem. Odeslání → volání **verify_person_access(token, heslo)**. Při úspěchu uložení session (localStorage) a načtení dat.
